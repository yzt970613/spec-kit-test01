...
- **Stack Fit**: Explain how this feature uses TypeScript, Vue 3 `<script setup>`,
  Less, and Tailwind CSS.
- **Plugin Impact**: List any new dependency, plugin, or tool required by this
  feature. If none, explicitly state "None".
- **Scope Guard**: Confirm the feature remains suitable for a frontend demo and
  does not require a new primary framework or unjustified backend expansion.
...
- **FR-006**: System MUST keep new frontend implementation within TypeScript,
  Vue 3 `<script setup>`, Less, and Tailwind CSS unless an exception is approved
  in the plan.
- **FR-007**: System MUST document any added plugin or tooling dependency and
  the user-visible reason it is needed.
